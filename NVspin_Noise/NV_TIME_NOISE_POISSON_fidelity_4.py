...
for x in range(count):                                       #반복 횟수 지정
    ...
    for y in tqdm(range(seccount)):                                 #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        costf = state_fidelity(real, deft1)
    ...
    costf = state_fidelity(real, desit)
    result = optimize.shgo(problem, bounds = bounds, iters = 5, options={'ftol': tol, 'xtol' : tol})
    output1.append(["Case" + str(x + 1) + "Fin", result['x'][0], result['x'][1], trace_time[0], ideal[0], ideal[1], ideal[2], deftl1[0], deftl1[1], deftl1[2], costf]) 
