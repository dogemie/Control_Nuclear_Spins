...
for x in range(count):                                       #반복 횟수 지정
    ...
    for y in tqdm(range(seccount)):                                 #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        idnoise = [np.trace(Rx(ns[y]) * idden * Sx()), np.trace(Ry(ns[y]) * idden * Sy()), np.trace(Rz(ns[y]) * idden * Sz())]
        print(idnoise)
        trace_time = [0, 5]
        repeat = repeat + 1
        ...
        result1 = optimize.shgo(problem, bounds = bounds, iters = 7, options={'ftol': tol, 'xtol' : tol})
