...
#target state는 idden 변수를 통해 변경할 수 있습니다.
# %%
...
def problem(deg):
    ...
    x_id = np.trace(idden*Sx() * Rx(ns[y]))                                 # target state의 Sigma X projection
    y_id = np.trace(idden*Sy() * Ry(ns[y]))                                 # target state의 Sigma Y projection
    z_id = np.trace(idden*Sz() * Rz(ns[y]))                                 # target state의 Sigma Z projection
...
for x in range(count):                                       #반복 횟수 지정
    ...
    tempTheta = 0
    tempPhi = 0
    ns = np.random.poisson(lam=25, size=seccount)/25
    for y in tqdm(range(seccount)):                                 #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        # ns = np.random.uniform(-1,1)
        # result1 = scipy.optimize.minimize(problem,deg,bounds=bounds,method="Nelder-Mead", options = {'xatol' : tol, 'fatol' : tol })        #Powell 최적화
        # result1 = optimize.shgo(problem, bounds = bounds, iters = 10, options={'ftol': tol, 'xtol' : tol})
        result1 = scipy.optimize.differential_evolution(problem, bounds, tol = tol, disp = False)
        ...
        costf = state_fidelity(idden, deft1)
    ...
    deftl1 = [np.trace(desit*Sx()),np.trace(desit*Sy()),np.trace(desit*Sz())]
    result = optimize.shgo(problem, bounds = bounds, iters = 5, options={'ftol': tol, 'xtol' : tol})
    output1.append(["Case" + str(x + 1) + "Fin", temTh, temPh, "X", ideal[0], ideal[1], ideal[2], deftl1[0], deftl1[1], deftl1[2], costf]) 
