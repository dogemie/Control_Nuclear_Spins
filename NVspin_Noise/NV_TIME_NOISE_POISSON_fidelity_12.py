...
def makeNoise(array, noiseSet):
    ...
    arre[0] = arral[0] + random.uniform(-0.1, 0.1)
    arre[1] = arral[1] + random.uniform(-0.1, 0.1)
    arre[2] = arral[2] + random.uniform(-0.1, 0.1)
...
def problem(deg):
    ...
    # # i_m = np.trace(rho_measure*I())                          # Identity projection
    # arre = [x_m, y_m, z_m]
    xyz = makeNoise(rho_measure, noisy)
    x_m = xyz[0]
    y_m = xyz[1]
    z_m = xyz[2]
...
#     #make noise with gaussian distribution
...
for zz in range(1):
    ...
    for x in range(count):                                       #반복 횟수 지정
        ...
        tempTheta = 0
        tempPhi = 0
        mu = 0 # mean and standard deviation of the normal distribution
        sigma = zz * 0.1
        ...
        for y in tqdm(range(seccount)):                                 #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
            ...
            # noisy[1] = (1 + random.uniform(-0.8, 1.4))
            # noisy[2] = (1 + random.uniform(-0.8, 1.4))
    ...
    fin1.rename(columns={0:"Case", 1:'Theta', 2: 'Phi', 3: 'X', 4: 'Y', 5: 'Z', 6: 'err', 7: 'temTh', 8: 'temPh', 9: 'cost', 10: 'fev'}, inplace=True)
    ...
    fin1.to_csv("C:/Users/Administrator/Dogyeom(2023.01.01)/KIST_intern/Task1/Control_Nuclear_Spins/NVspin_Noise/Test_Result_" + str(round(zz * 0.1, 1)) + "_" + printdate + '.csv', index=false)
