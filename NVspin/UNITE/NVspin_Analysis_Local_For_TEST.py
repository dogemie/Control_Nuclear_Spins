...
count = 50 #최적화 정도를 확인하기 위한 반복 작업 횟수
...
for x in range(count):
    ...
    #result11 = scipy.optimize.minimize(problem2,deg,bounds=bounds,method="Nelder-Mead")
    end = time.time()
    final = end - start
    ...
    # deft11 = degree(result11['x'][0], result11['x'][1]) #최적화된 값으로 density matrix를 구한다.
    # car11 = ((idden.data[0, 0] - deft11[0, 0])**2 + (idden.data[0, 1] - deft11[0, 1])**2 + (idden.data[1, 1] - deft11[1, 1])**2)**(1/2)
    # if(car > car11):
    #     print("car11")
    #     car = car11
    # else:
    ...
    #result22 = scipy.optimize.minimize(problem2,deg,bounds=bounds, method="Powell")
    end = time.time()
    final = end - start
    ...
    # deft22 = degree(result22['x'][0], result22['x'][1])
    # car22 = ((idden.data[0, 0] - deft22[0, 0])**2 + (idden.data[0, 1] - deft22[0, 1])**2 + (idden.data[1, 1] - deft22[1, 1])**2)**(1/2)
    # if(car > car22):
    #     print("car22")
    #     car = car22
    # else:
...
# plt.plot(temp, temp3, color='green', marker='o', label='Test3', linestyle='dashed', linewidth=2, markersize=12)
# plt.plot(temp, temp4, color='yellow', marker='o', label='Test4', linestyle='dashed', linewidth=2, markersize=12)
