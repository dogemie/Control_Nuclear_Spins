...
for x in range(count):                                         #반복 횟수 지정
    ...
    for y in range(0, seccount):                                #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        if var1 < vastand:                                      #최적화 정도가 기준보다 작을 경우
            ...
            output1.append(["Case" + str(x + 1), "Nelder-Mead", result1['x'], ideal, deftl1, idden.data, var1])                                #측정 값 저장
            success = success + 1
            break
...
fin1.rename(columns={0:"Case", 1:"Used Algorithm", 2:'Theta, Phi', 3: "Target state Projection", 4: "Optimized state Projection", 5: "idden", 6: "Cost"}, inplace=True)
