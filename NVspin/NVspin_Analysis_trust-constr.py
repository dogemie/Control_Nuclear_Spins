...
while true:
    result = scipy.optimize.minimize(problem,deg,bounds=bounds,method='trust-constr',options={'xtol': xx,'gtol': ff})
    output.append(result)
    aaa = output[count - 1]['fun']          #Temp Memory to compate output 'fun' value
    if(count == 10 and aaa > 2):
        print("minimize fail")
        break
...
print(date)
fin.to_csv("C:/Users/Administrator/2023.01.01/KIST_intern/Task1/Control_Nuclear_Spins/NVspin/trust-constr/trust-constr_result_" + printdate + '.csv', index=false)
...
def function(count):
    ...
    bo_temp = bounds
    while true:
        result = scipy.optimize.minimize(pro,de_temp,bounds=bo_temp,method='trust-constr',options={'xtol': xx,'gtol': ff})
        output.append(result)
        aaa = output[count - 1]['fun']          #Temp Memory to compate output 'fun' value
        if(count > 10 and aaa >= 1):
            print("refind minimum")
            count = count + 1
            return 1
        if(aaa < 1 and float(aaa) >= float(bbb) and float(aaa) >= float(ccc)):
            print("minimize clear")
            return 0
        if(count >= 100):
            print("minimize fail")
