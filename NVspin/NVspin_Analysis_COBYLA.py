...
while true:
    result = scipy.optimize.minimize(problem,deg,bounds=bounds,method='COBYLA',options={'tol': xx,'catol': ff})
    output.append(result)
...
ccc = 100                                   #이전의 이전의 cost function값을 저장하기 위한 변수
while true:
    result = scipy.optimize.minimize(problem,deg,bounds=bounds,method='COBYLA')
    output.append(result)
    aaa = output[count - 1]['fun']          #Temp Memory to compate output 'fun' value
...
print(date)
fin.to_csv("C:/Users/Administrator/2023.01.01/KIST_intern/Task1/Control_Nuclear_Spins/NVspin/COBYLA/COBYLA_result_" + printdate + '.csv', index=false)
