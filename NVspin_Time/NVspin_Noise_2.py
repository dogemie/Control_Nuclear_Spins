...
ido = np.zeros(3, dtype='complex_')
ide = np.zeros(3, dtype='complex_')
...
def problem(deg):
    ...
    # i_m = np.trace(rho_measure*I())                          # Identity projection
    #x_id,y_id,z_id는 주어진 target state를 계산해낸 값(이론값)
    x_id = ide[0]                                 # target state의 Sigma X projection
    y_id = ide[1]                                 # target state의 Sigma Y projection
    z_id = ide[2]                                 # target state의 Sigma Z projection
    # i_id = np.trace(idden*I())                               # target state의 Identity projection
    # 실험값과 이론값의 비교 costfunction 반환
...
for x in tqdm(range(count)):                                         #반복 횟수 지정
    ...
    for y in range(0, seccount):                                #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        result1 = optimize.shgo(problem, bounds = bounds, iters = 4, options={'ftol': tol, 'xtol' : tol})
        ...
        output1.append(["Case" + str(x + 1), result1['x'][0], result1['x'][1], ideal, deftl1, ide, result1['fun']])                                #측정 값 저장
        success = success + 1
...
fin1.rename(columns={0:"Case", 1:'Theta', 2: 'Phi', 3: 'init', 4: 'trace', 5: 'noise'}, inplace=True)
