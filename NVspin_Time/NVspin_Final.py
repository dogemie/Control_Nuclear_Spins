...
시간에 따른 오차를 보정하여 문제를 해결할 수 있도록 하였습니다. By Dogyeom 2023.03.30
'''
...
# TODO : deg(초기값)에 z축에 대한 시간에 따른 회전값을 추가 해야함.

bounds = [(0, pi),(0,2*pi), (0, 20)]                                     #theta와 phi의 범위
deg = [(np.pi/180)*random.uniform(0,180),(np.pi/180)*random.uniform(0,360), 0] #초기값을 넣는 랜덤변수
...
for x in tqdm(range(count)):                                         #반복 횟수 지정
    ...
    for y in range(0, seccount):                                #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        if var1 < vastand:                                      #최적화 정도가 기준보다 작을 경우
            ...
            output1.append(["Case" + str(x + 1), result1['x'], dat[0][0].real, dat[0][0].imag, dat[0][1].real, dat[0][1].imag, dat[1][0].real, dat[1][0].imag, dat[1][1].real, dat[1][1].imag])                                #측정 값 저장
            success = success + 1
            break
...
fin1.rename(columns={0:"Case", 1:'Theta, Phi, time', 2: '00R', 3: '00I', 4: '01R', 5: '01I', 6: '10R', 7: '10I', 8: '11R', 9: '11I'}, inplace=True)
