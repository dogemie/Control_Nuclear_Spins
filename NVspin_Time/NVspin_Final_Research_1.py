...
for x in tqdm(range(count)):                                         #반복 횟수 지정
    ...
    for y in range(0, seccount):                                #측정 횟수 지정 같은 작업을 여러번 진행할 경우를 대비하여 반복문 사용
        ...
        result1 = optimize.shgo(problem, bounds = bounds, iters = 5, options={'ftol': tol, 'xtol' : tol, 'disp': True})
        ...
        if var1 < vastand:                                      #최적화 정도가 기준보다 작을 경우
            ...
            output1.append(["Case" + str(x + 1), result1['x'][0], result1['x'][1], result1['x'][2], ideal, deftl1, result1['fun']])                                #측정 값 저장
            success = success + 1
            break
